...
static PTKToken_ST isIdentifierOrKeyword (
	FILE* source_code,
	const PTKToken_ST reserved_words[],
	unsigned line
) {
    if (source_code && reserved_words) {
        ...
        token_return = ptkGetToken(word_return, reserved_words);
        if (!ptkIsNullToken(token_return))
             return ptkDefToken(token_return.id, NULL, line);
        else return ptkDefToken(PTK_ID, word_return, line);
...
static PTKToken_ST isDecimalOrFloatConstant (
	FILE* source_code,
	unsigned line
) {
    if (source_code) {
...
static PTKToken_ST isBinaryConstant (FILE* source_code, unsigned line) {
    if (source_code) {
        char constant_return[51] = "\0";
        char character = getc(source_code);
        int flag_error = 0, i = 0;
        ...
        if (flag_error)
             return
                ptkDefToken(PTK_ERROR_BINARY_CONSTANT, constant_return, line);
        else return ptkDefToken(PTK_BIN_INT_VAL, constant_return, line);
    } else exit(1); /* trocar por um erro apropriado */
}
...
static PTKToken_ST isHexadecimalConstant (FILE* source_code, unsigned line) {
    if (source_code) {
        ...
        if (flag_error)
            ...
            return ptkDefToken(PTK_HEXADECIMAL_INT_VAL, constant_return, line);
...
PTKToken_ST pscGetToken (FILE* source_code) {
    ...
    if (source_code) {
        ...
        switch (character) {
        case EOF: return ptkDefToken(PTK_END_OF_FILE, NULL, line);
        ...
        case '.':
            ...
            case '.':
                switch (getc(source_code)) {
                case '.': return ptkDefToken(PTK_RETICENCE, NULL, line);
                default :
                    fseek(source_code, -1, SEEK_CUR);

                    return ptkDefToken(PTK_OP_TO, NULL, line);
                }
            default :
                fseek(source_code, -1, SEEK_CUR);
        ...
        case '&': return ptkDefToken(PTK_OP_AND, NULL, line);

        case '|': return ptkDefToken(PTK_OP_OR, NULL, line);

        case ':':
            switch (getc(source_code)) {
            case ':': return ptkDefToken(PTK_OP_ECP, NULL, line);
        ...
        case '$': return ptkDefToken(PTK_DOLLAR, NULL, line);

        case ';': return ptkDefToken(PTK_SEMICOLON, NULL, line);

        case '[': return ptkDefToken(PTK_LEFT_BRACKET, NULL, line);

        case ']': return ptkDefToken(PTK_RIGHT_BRACKET, NULL, line);

        case '(': return ptkDefToken(PTK_LEFT_PARENTHESIS, NULL, line);

        case ')': return ptkDefToken(PTK_RIGHT_PARENTHESIS, NULL, line);

        case '{': return ptkDefToken(PTK_LEFT_BRACE, NULL, line);

        case '}': return ptkDefToken(PTK_RIGHT_BRACE, NULL, line);
        ...
        default  :
            if (isdigit(character)) {
                ...
                return isIdentifierOrKeyword(
					source_code, PTK_RESERVED_WORDS, line
				);
